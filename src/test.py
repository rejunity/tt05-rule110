...
async def test_rule110_automata(dut):
    ...
    # for i in range(32):
        await ClockCycles(dut.clk, 1)
        print(
            # dut.tt_um_rejunity_rule110_uut.write_enable.value, dut.tt_um_rejunity_rule110_uut.halt.value,
            #str(dut.tt_um_rejunity_rule110_uut.cells_dt.value).replace('0', ' ').replace('1', '@'),
            str(dut.tt_um_rejunity_rule110_uut.cells.value).replace('0', ' ').replace('1', '@'),
            dut.ui_in.value,
            dut.uio_in.value,
            dut.uo_out.value)
